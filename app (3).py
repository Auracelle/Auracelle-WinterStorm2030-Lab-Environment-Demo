...
tab_names.append("📜 Shared Move Feed")
...
with tab_log:
    ...
    st.write("A persistent, auditable record of participant, Agentic AI, OSINT-tagged, and arbitration actions. Most recent moves appear first.")
    ...
    if not raw_log:
        ...
        for entry in reversed(raw_log):
            ...
            items_html += f"""
            <div style="border:1px solid var(--line);border-left:3px solid {color};border-radius:10px;padding:.7rem .85rem;margin-bottom:.55rem;background:rgba(13,42,57,.45);">
              <div style="font-size:.72rem;color:var(--muted);letter-spacing:.03em;">Turn {entry.get('turn','?')} · {entry.get('ts','')} · {callsign_e}</div>
              <div style="margin:.25rem 0;"><span style="color:{color};font-weight:700;font-size:.78rem;text-transform:uppercase;letter-spacing:.05em;">{src}</span>
                <strong style="color:var(--text);margin-left:.5rem;">{actor}</strong>
                <span style="color:var(--muted);"> — {move_type}</span></div>
              <div style="color:var(--muted);font-size:.9rem;">{detail}</div>
              <div style="font-size:.75rem;color:{'#ff727f' if delta>0 else '#7ee2a8' if delta<0 else 'var(--muted)'};margin-top:.25rem;">Cap Gap impact: {delta_str}</div>
            </div>"""
        st.markdown(f'<div style="max-height:560px;overflow-y:auto;padding-right:.4rem;">{items_html}</div>', unsafe_allow_html=True)
