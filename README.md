...
**Note:** this was originally split into `app.py` + `pages/simulation.py` (matching Bach's structure),
but was consolidated into a single `app.py` after repeated `st.switch_page` deploy failures caused by
`pages/simulation.py` not being present in the deployed repo. One file removes that entire class of
path/deploy mismatch — worth the small departure from Bach's pattern for deployment reliability.
