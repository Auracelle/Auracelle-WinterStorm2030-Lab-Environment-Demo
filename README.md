...
- **Decision-Maker (breadth):** read-only Scenario Setup, Concession Engine, Shared Move Log.
- **Decision-Support (full depth):** adds editable Scenario Setup and the Arbitration Track
  (the multi-stage escalation ladder — more procedural/analytical than a quick decision).

This is an adapted mapping, not a full port of the HTML build's DM/DS split. The HTML
version differentiates using tools this Streamlit build doesn't have yet — Cognitive
Warfare, Narrative Analysis, Governance Lab, OSINT/Hormuz calibration (see Scope note
below). If those get ported later, the DM/DS split should be revisited to route them
to Decision-Support rather than leaving everything in one undifferentiated tab.
