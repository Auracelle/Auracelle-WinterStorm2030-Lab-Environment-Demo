...
SECONDARY_OBJECTIVES = {
    ...
    "evidence_tags": {"label": "Tag at least 2 real OSINT events into the Article 4 Stress Test Log"},
    "treaty_scoring": {"label": "Score at least 1 treaty or policy document in the Governance Lab"},
    "arbitration_resolution": {"label": "Resolve at least 1 dispute through the Arbitration Track (not just raw concessions)"},
}
...
def evaluate_secondary_objectives(state):
    ...
    needs_data = any(k in keys for k in ("evidence_tags", "treaty_scoring", "arbitration_resolution"))
    ...
    for key in keys:
        ...
        elif key == "evidence_tags":
            results[key] = len([e for e in data.get("article4_log", []) if e.get("source") == "REAL"]) >= 2
        elif key == "treaty_scoring":
            results[key] = len(data.get("govlab_docs", [])) >= 1
        elif key == "arbitration_resolution":
            resolved = [c for c in data.get("arbitration", []) if str(c.get("status", "")).startswith("CLOSED")]
            results[key] = len(resolved) >= 1
    return results
...
def issue_ruling(row, callsign):
    ...
    pattern = ARBITRATION_DESIGN_PATTERNS[result["ruling"]]
    result["summary"] = (
