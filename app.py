...
ADVERSARIAL_VECTORS = {
    "infra": {"icon": "⊙", "label": "Critical Infrastructure", "sub": "Cable // Energy // PNT",
              "help": "Undersea cables, energy links, or PNT (positioning/navigation/timing) systems are interfered with or sabotaged."},
    "disinfo": {"icon": "≈", "label": "Disinformation Op", "sub": "Narrative // IIC Vector",
                "help": "Coordinated narrative campaign targeting alliance cohesion or public trust (IIC — Information Integrity Composite)."},
    "auto": {"icon": "▣", "label": "Autonomous Systems", "sub": "UxV // ASI Vector",
             "help": "Unmanned vehicles (UxV) or autonomous sensing infrastructure (ASI) probe or contest a boundary/asset."},
    "energy": {"icon": "⚡", "label": "Energy Coercion", "sub": "Gas // Oil // Grid",
               "help": "Gas, oil, or grid leverage used as pressure against a Blue Team actor's energy dependence."},
    "hybrid": {"icon": "⬡", "label": "Compound Hybrid", "sub": "Multi-Domain",
               "help": "Two or more vectors occur together (e.g. cyber probe + narrative push), designed to saturate Blue Team attention."},
    "legal": {"icon": "⚖", "label": "Legal / Sovereignty", "sub": "Grey Zone",
              "help": "A grey-zone move that exploits treaty or legal ambiguity (e.g. Article 9, EEZ boundary) rather than a physical or cyber act."},
}
INTENSITY_LEVELS = {
...
with tab_scenario:
    ...
    if is_dm:
        ...
        vec = state.get("adversarial_vector")
        st.markdown(f"**Adversarial Vector:** {ADVERSARIAL_VECTORS[vec]['label'] if vec else '_(not set)_'}")
        st.markdown(f"**Intensity:** {INTENSITY_LEVELS.get(state.get('intensity','medium'))['label']}")
        st.markdown(f"**Blue Actors:** {', '.join(state.get('blue_actors', [])) or '_(none)_'}")
        st.markdown(f"**Red Actors:** {', '.join(state.get('red_actors', [])) or '_(none)_'}")
        ...
        st.write("Select the primary adversarial action this scenario centers on.")
        ...
        for i, vk in enumerate(vec_keys):
            ...
            with vcols[i % 3]:
                st.markdown(
                    f"<div class='ws-domain-card' style='text-align:center;padding:.7rem .4rem;margin-bottom:.5rem;' title='{html.escape(v['help'])}'>"
                    f"<div style='font-size:1.5rem;'>{v['icon']}</div>"
                    f"<div style='font-weight:700;font-size:.8rem;margin:.25rem 0 .1rem;'>{v['label']}</div>"
                    f"<div style='font-size:.68rem;color:var(--muted);'>{v['sub']}</div></div>",
                    unsafe_allow_html=True)
        selected_vector = st.radio("Confirm vector", options=vec_keys,
                                    format_func=lambda k: f"{ADVERSARIAL_VECTORS[k]['icon']} {ADVERSARIAL_VECTORS[k]['label']}",
                                    index=vec_keys.index(state.get("adversarial_vector")) if state.get("adversarial_vector") in vec_keys else 0,
                                    horizontal=True, key="vector_radio")
        st.caption(ADVERSARIAL_VECTORS[selected_vector]["help"])
        ...
        acols = st.columns(2)
        blue_sel = acols[0].multiselect("Blue Team actors", options=ALL_BLUE_ACTORS, default=state.get("blue_actors", ALL_BLUE_ACTORS[:4]))
        red_sel = acols[1].multiselect("Red Team actors", options=ALL_RED_ACTORS, default=state.get("red_actors", ["Russia"]))
...
if not is_dm:
    ...
    with tab_govlab:
        ...
        st.markdown("<div class='ws-callout-panel'><strong style='color:#edf7fa'>Reference workspace</strong> — upload or paste a treaty, policy, or legal agreement for indicative assessment against NATO doctrine and this session's evidence.</div>", unsafe_allow_html=True)
        st.write("Upload or paste a treaty, policy, or legal agreement (e.g. the Washington Naval Treaty) for assessment against NATO doctrine and this session's own generated evidence. Scoring is rules-based lexical overlap — labeled indicative, not a legal or AI judgment.")
