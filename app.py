...
.stButton>button,.stFormSubmitButton>button{min-height:2.75rem;background:linear-gradient(135deg,#59ece3,#25bfb8)!important;color:#032024!important;font-weight:800!important;border:1px solid rgba(175,255,250,.65)!important;border-radius:10px!important;padding:.65rem 1rem!important;box-shadow:0 8px 20px rgba(12,174,165,.18),inset 0 1px 0 rgba(255,255,255,.45)!important;transition:.16s ease!important}.stButton>button:hover,.stFormSubmitButton>button:hover{transform:translateY(-1px);filter:brightness(1.08);box-shadow:0 12px 28px rgba(12,174,165,.26)!important}.stButton>button:active,.stFormSubmitButton>button:active{transform:translateY(1px)}
...
.stTabs [data-baseweb="tab-list"]{gap:.32rem;padding:.35rem;border:1px solid var(--line);border-radius:13px;background:rgba(4,18,29,.72);overflow-x:auto}.stTabs [data-baseweb="tab"]{height:2.75rem;border-radius:9px;padding:0 .9rem;color:var(--muted);font-weight:650}.stTabs [aria-selected="true"]{background:rgba(85,229,222,.12)!important;color:var(--cyan2)!important}.stTabs [data-baseweb="tab-highlight"]{background-color:var(--cyan)}
...
with tab_capgap:
    ...
    st.dataframe(domain_df.style.bar(subset=["Gap %"], vmin=0, vmax=100), use_container_width=True, hide_index=True)
...
with tab_osint:
    st.markdown("<div class='ws-section-label'>Open-Source Intelligence & Event Calibration</div>", unsafe_allow_html=True)
    st.write("Real event data from the GDELT Project — free, unclassified, no API key, and no generative-AI dependency. Events may be routed into the Concession Engine, Article 4 stress-test log, or the non-scoring Hormuz calibration baseline.")
    ocols = st.columns(3)
...
with tab_concession:
    st.markdown("<div class='ws-section-label'>Contested Negotiation Table · Concession Engine</div>", unsafe_allow_html=True)
    st.write("Model Access, Resource, and Narrative-Legal concessions. Every decision is written immediately to the shared move log and becomes visible to distributed participants on refresh.")
...
if not is_dm:
    with tab_cogwar:
        st.markdown("<div class='ws-section-label'>Cognitive Warfare · Narrative Pressure Analysis</div>", unsafe_allow_html=True)
    ...
    with tab_arbitration:
        st.markdown("<div class='ws-section-label'>Grey-Zone Dispute Resolution & Arbitration Track</div>", unsafe_allow_html=True)
        st.write("Boundary Waters / Columbia River / Canterbury model — a standing escalation ladder designed to contain strategic friction before it becomes an alliance-level crisis.")
    ...
    with tab_govlab:
        st.markdown("<div class='ws-section-label'>Governance Lab · Treaty & Precedent Library</div>", unsafe_allow_html=True)
        st.write("Treaty and precedent reference material grounds scenario decisions without altering live game state. Use these analogues to evaluate Article 4 thresholds, infrastructure disputes, escalation ladders, and negotiated continuity mechanisms.")
...
with tab_log:
    st.markdown("<div class='ws-section-label'>Asynchronous Shared Decision Record</div>", unsafe_allow_html=True)
    st.write("Every participant and Agentic AI move appears here, most recent first. This persistent decision record supports distributed play, facilitation, auditability, and after-action review.")
    df = load_log()
    if df.empty:
