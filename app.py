...
/* 2020s interaction system — tactile cards, glass controls, no exposed radio circles */
:root{--ease:cubic-bezier(.22,1,.36,1);--focus:0 0 0 3px rgba(122,231,244,.16),0 0 28px rgba(63,169,255,.18)}
*{scrollbar-width:thin;scrollbar-color:rgba(122,231,244,.34) rgba(3,16,26,.34)}
*::-webkit-scrollbar{width:9px;height:9px}*::-webkit-scrollbar-track{background:rgba(3,16,26,.34)}*::-webkit-scrollbar-thumb{background:linear-gradient(180deg,rgba(122,231,244,.42),rgba(63,169,255,.25));border-radius:999px;border:2px solid rgba(3,16,26,.5)}

/* Remove old browser-native radio affordances if a future section introduces one. */
div[role="radiogroup"]{display:flex;gap:.55rem;flex-wrap:wrap}div[role="radiogroup"] label{position:relative;flex:1 1 220px;margin:0!important;padding:.78rem .92rem!important;border:1px solid var(--line)!important;border-radius:14px!important;background:linear-gradient(145deg,rgba(10,36,52,.76),rgba(5,22,34,.64))!important;box-shadow:inset 0 1px 0 rgba(255,255,255,.035);transition:transform .18s var(--ease),border-color .18s ease,box-shadow .18s ease,background .18s ease}div[role="radiogroup"] label:hover{transform:translateY(-2px);border-color:rgba(122,231,244,.6)!important;box-shadow:0 12px 25px rgba(0,0,0,.22),0 0 18px rgba(63,169,255,.12)}div[role="radiogroup"] input{position:absolute!important;opacity:0!important;pointer-events:none!important}div[role="radiogroup"] label:has(input:checked){border-color:var(--cyan)!important;background:linear-gradient(145deg,rgba(18,72,91,.82),rgba(8,39,57,.76))!important;box-shadow:var(--focus),inset 0 1px 0 rgba(255,255,255,.08)}

/* Segmented controls become responsive selection cards instead of compact legacy pills. */
div[data-testid="stSegmentedControl"] [role="radiogroup"]{display:grid!important;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));width:100%;gap:.6rem!important}div[data-testid="stSegmentedControl"] label{display:flex!important;align-items:center!important;justify-content:center!important;text-align:center!important;white-space:normal!important;line-height:1.25!important;min-height:3.25rem!important;margin:0!important;padding:.78rem .9rem!important;border:1px solid rgba(122,231,244,.2)!important;border-radius:14px!important;background:linear-gradient(145deg,rgba(10,35,51,.82),rgba(5,22,34,.68))!important;box-shadow:inset 0 1px 0 rgba(255,255,255,.035)!important;transition:transform .18s var(--ease),border-color .18s ease,box-shadow .18s ease,background .18s ease!important}div[data-testid="stSegmentedControl"] label:hover{transform:translateY(-2px);border-color:rgba(122,231,244,.62)!important;box-shadow:0 12px 24px rgba(0,0,0,.2),0 0 18px rgba(63,169,255,.12)!important}div[data-testid="stSegmentedControl"] label[aria-checked="true"],div[data-testid="stSegmentedControl"] label[data-checked="true"]{transform:translateY(-1px);border-color:var(--cyan)!important;background:linear-gradient(145deg,rgba(20,82,102,.88),rgba(8,41,61,.82))!important;box-shadow:var(--focus),inset 0 1px 0 rgba(255,255,255,.1)!important}div[data-testid="stSegmentedControl"] p{white-space:normal!important;font-size:.84rem!important}

/* Inputs and selectors */
.stTextInput>div>div,.stTextArea>div>div,.stNumberInput>div>div,div[data-baseweb="select"]>div{border:1px solid rgba(122,231,244,.22)!important;background:linear-gradient(145deg,rgba(6,26,40,.96),rgba(5,20,32,.9))!important;border-radius:14px!important;box-shadow:inset 0 1px 0 rgba(255,255,255,.03)!important;transition:border-color .18s ease,box-shadow .18s ease,transform .18s var(--ease)!important}.stTextInput>div>div:focus-within,.stTextArea>div>div:focus-within,.stNumberInput>div>div:focus-within,div[data-baseweb="select"]>div:focus-within{border-color:var(--cyan)!important;box-shadow:var(--focus)!important;transform:translateY(-1px)}
[data-baseweb="popover"]>div{background:rgba(4,20,31,.98)!important;border:1px solid rgba(122,231,244,.25)!important;border-radius:14px!important;box-shadow:0 24px 55px rgba(0,0,0,.48)!important;backdrop-filter:blur(18px)}[role="option"]{border-radius:9px!important;margin:3px 5px!important;width:calc(100% - 10px)!important}[role="option"]:hover,[aria-selected="true"][role="option"]{background:rgba(122,231,244,.12)!important}

/* Multiselect values become modern removable chips. */
[data-baseweb="tag"]{border-radius:999px!important;background:linear-gradient(135deg,rgba(122,231,244,.22),rgba(63,169,255,.16))!important;border:1px solid rgba(122,231,244,.35)!important;box-shadow:inset 0 1px 0 rgba(255,255,255,.05)!important}[data-baseweb="tag"] span{color:var(--ice)!important;font-weight:700!important}

/* Toggle, file uploader, download buttons and utility controls */
[data-testid="stToggle"]{padding:.66rem .82rem;border:1px solid var(--line);border-radius:14px;background:rgba(7,27,41,.48)}[data-testid="stToggle"] label{gap:.6rem!important}[data-testid="stToggle"] [role="switch"]{box-shadow:inset 0 0 0 1px rgba(255,255,255,.12),0 4px 12px rgba(0,0,0,.2)!important}
[data-testid="stFileUploaderDropzone"]{min-height:145px!important;border:1px dashed rgba(122,231,244,.42)!important;border-radius:18px!important;background:radial-gradient(circle at 50% 0%,rgba(122,231,244,.1),transparent 55%),rgba(6,25,38,.62)!important;transition:transform .18s var(--ease),border-color .18s ease,box-shadow .18s ease!important}[data-testid="stFileUploaderDropzone"]:hover{transform:translateY(-2px);border-color:var(--cyan)!important;box-shadow:0 16px 34px rgba(0,0,0,.25),0 0 25px rgba(63,169,255,.12)!important}
.stDownloadButton>button{min-height:2.65rem!important;border-radius:11px!important;border:1px solid rgba(122,231,244,.28)!important;background:rgba(9,33,49,.68)!important;color:var(--cyan2)!important;font-weight:750!important;box-shadow:none!important}.stDownloadButton>button:hover{border-color:var(--cyan)!important;background:rgba(122,231,244,.12)!important;transform:translateY(-1px)}

/* Navigation and content surfaces */
.stTabs [data-baseweb="tab-list"]{position:sticky;top:.35rem;z-index:40;backdrop-filter:blur(18px);box-shadow:0 10px 30px rgba(0,0,0,.18)}.stTabs [data-baseweb="tab"]{transition:background .18s ease,color .18s ease,transform .18s var(--ease)!important}.stTabs [data-baseweb="tab"]:hover{background:rgba(122,231,244,.07)!important;color:var(--ice)!important;transform:translateY(-1px)}
div[data-testid="stExpander"]{transition:border-color .18s ease,box-shadow .18s ease,transform .18s var(--ease)}div[data-testid="stExpander"]:hover{border-color:rgba(122,231,244,.34)!important;box-shadow:0 12px 28px rgba(0,0,0,.14)}details summary{padding:.25rem .15rem!important;font-weight:750!important}
[data-testid="stDataFrame"]{box-shadow:0 12px 30px rgba(0,0,0,.16)}
[data-testid="stAlert"]{backdrop-filter:blur(10px);box-shadow:inset 0 1px 0 rgba(255,255,255,.03)}

/* Command buttons get clear hierarchy and comfortable touch targets. */
.stButton>button,.stFormSubmitButton>button{letter-spacing:.01em!important;will-change:transform;transition:transform .18s var(--ease),filter .18s ease,box-shadow .18s ease!important}.stButton>button:focus-visible,.stFormSubmitButton>button:focus-visible,.stDownloadButton>button:focus-visible{outline:none!important;box-shadow:var(--focus)!important}.stButton>button[kind="secondary"],.stButton>button[data-testid="stBaseButton-secondary"]{background:linear-gradient(145deg,rgba(11,38,55,.9),rgba(6,25,38,.85))!important;color:var(--ice)!important;border:1px solid rgba(122,231,244,.24)!important;box-shadow:inset 0 1px 0 rgba(255,255,255,.035)!important}.stButton>button[kind="secondary"]:hover,.stButton>button[data-testid="stBaseButton-secondary"]:hover{border-color:var(--cyan)!important;background:linear-gradient(145deg,rgba(15,52,71,.95),rgba(8,32,47,.9))!important;box-shadow:0 12px 24px rgba(0,0,0,.22),0 0 18px rgba(63,169,255,.1)!important}

/* Subtle motion, disabled for users who prefer reduced motion. */
.ws-hero,.ws-panel,.ws-brief,.ws-callout-panel{animation:ws-enter .48s var(--ease) both}@keyframes ws-enter{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:translateY(0)}}
...
DEFAULT_DATA = {
    "state": {
        ...
        "game_over": False, "game_result": None, "game_result_text": "",
        "domains": [
...
def score_document(doc_text, doc_title, callsign, live_query="", live_timespan="7d"):
    ...
    for art in article_matches[:4]:
        ...
        if matched_events:
            ...
            gap_findings.append(
                ...
                f"{len(matched_events)} live event(s) show this being tested right now: {titles}."
            )
        else:
            gap_findings.append(
                f"**{art['article']}** {strength_tag} — {meta['note']} "
                f"No live event in this pull corroborates it, but the language assessment holds independent of current events."
            )
    ...
    coverage_score = min(100, len(article_matches) * 6 + len(covered_violations) * 8 + len(precedent_matches) * 5 + min(session_hits, 10) * 2 + min(len(live_events), 5) * 2)
...
def evaluate_secondary_objectives(state):
    ...
    results = {}
    for key in state.get("secondary_objectives", []):
        if key == "cohesion":
...
if not st.session_state["authenticated"]:
    ...
    with st.form("login_form"):
        ...
        submit = st.form_submit_button("Enter Decision Environment →")
...
def render_turn_status_banner(state):
    ...
    if state.get("turn_action_taken"):
        st.markdown(
            ...
            f"<div style='margin-top:.5rem;font-size:.85rem;'>🧭 <strong>Turn Recommendation:</strong> {recs_html}</div></div>",
            unsafe_allow_html=True)
    else:
        st.markdown(
...
with tab_scenario:
    ...
    st.write("This is what turns the Cap Gap number into something you're actually trying to achieve, not just watch move. Pick one primary objective and up to three secondary objectives.")
    ...
    primary_labels = {k: PRIMARY_OBJECTIVES[k]["label"] for k in pri_keys}
    primary_by_label = {label: key for key, label in primary_labels.items()}
    selected_primary_label = st.segmented_control(
        "Primary Objective",
        options=list(primary_by_label.keys()),
        default=primary_labels[default_pri],
        key="primary_objective_cards",
    )
    primary_objective = primary_by_label.get(selected_primary_label, default_pri)
    st.caption(PRIMARY_OBJECTIVES[primary_objective]["description"])
    sec_keys = list(SECONDARY_OBJECTIVES.keys())
    secondary_objectives = st.multiselect("Secondary objectives · select up to three", options=sec_keys,
    ...
    doctrine = st.selectbox("Adversary doctrine", options=list(DOCTRINE_LABELS.keys()),
    ...
    intensity_labels = {k: f"{INTENSITY_LEVELS[k]['label']} — {INTENSITY_LEVELS[k]['sub']}" for k in int_keys}
    intensity_by_label = {label: key for key, label in intensity_labels.items()}
    default_intensity = state.get("intensity", "medium") if state.get("intensity", "medium") in int_keys else "medium"
    selected_intensity_label = st.segmented_control(
        "Intensity level",
        options=list(intensity_by_label.keys()),
        default=intensity_labels[default_intensity],
        key="intensity_cards",
    )
    selected_intensity = intensity_by_label.get(selected_intensity_label, default_intensity)
    ...
    with acols[0]:
        with st.container(key="blue_actor_box"):
            blue_sel = st.multiselect("Blue Team coalition", options=ALL_BLUE_ACTORS, default=state.get("blue_actors", ALL_BLUE_ACTORS[:4]))
    with acols[1]:
...
if not is_dm:
    ...
    with tab_govlab:
        ...
        upload_mode = st.segmented_control(
            ...
            default="✦ Paste text",
            key="govlab_input_mode",
        )
        upload_mode = "Paste text" if upload_mode != "⬆ Upload document" else "Upload file (.txt, .pdf, .docx)"
