...
# ══════════════════════════════════════════════════════════
# SUPABASE — shared, persistent state (this is what makes async multiplayer work)
# Uses a plain API key (anon key), not a downloadable service-account key file, so
# org policies blocking GCP service account key creation don't apply here.
# ══════════════════════════════════════════════════════════
@st.cache_resource
def get_client():
...
def load_state():
    client = get_client()
    res = client.table("ws_state").select("*").eq("id", 1).execute()
    if not res.data:
        st.error("ws_state row missing. Run supabase_setup.sql in the Supabase SQL Editor first.")
        st.stop()
    return res.data[0]
...
def save_state(state):
    ...
    payload = {k: state[k] for k in
               ["turn", "cap_gap", "concession_modifier", "narrative_modifier", "doctrine",
                "scenario_name", "scenario_desc", "treaty_flag", "win_win_active"]}
    client.table("ws_state").update(payload).eq("id", 1).execute()

def append_log(turn, callsign, actor, move_type, source, detail, cap_gap_delta):
    client = get_client()
    client.table("ws_log").insert({
        "turn": turn, "callsign": callsign, "actor": actor, "move_type": move_type,
        "source": source, "detail": detail, "cap_gap_delta": cap_gap_delta
    }).execute()

def load_log():
    client = get_client()
    res = client.table("ws_log").select("*").order("id", desc=True).execute()
    if not res.data:
...
def load_arbitration():
    client = get_client()
    res = client.table("ws_arbitration").select("*").order("id").execute()
    return pd.DataFrame(res.data) if res.data else pd.DataFrame(
        columns=["id", "case_id", "status", "violation", "asset", "stage", "opened_by", "opened_at"])

def open_arbitration(violation, asset, callsign):
    client = get_client()
    client.table("ws_arbitration").insert({
        "case_id": datetime.now().strftime("%Y%m%d%H%M%S"),
        "status": "OPEN", "violation": violation, "asset": asset,
        "stage": 1, "opened_by": callsign
    }).execute()

def update_arbitration(row_id, fields):
    client = get_client()
    client.table("ws_arbitration").update(fields).eq("id", row_id).execute()
