...
/* 2020s interaction polish */
[data-testid="stAppViewContainer"]{scroll-behavior:smooth}
*{scrollbar-width:thin;scrollbar-color:rgba(122,231,244,.42) rgba(4,18,29,.55)}
*::-webkit-scrollbar{width:10px;height:10px}*::-webkit-scrollbar-track{background:rgba(4,18,29,.55);border-radius:999px}*::-webkit-scrollbar-thumb{background:linear-gradient(180deg,rgba(122,231,244,.58),rgba(63,169,255,.42));border:2px solid rgba(4,18,29,.75);border-radius:999px}

.stTabs [data-baseweb="tab-list"]{
  position:sticky;top:.35rem;z-index:20;gap:.42rem;padding:.48rem;
  border:1px solid rgba(122,231,244,.22);border-radius:18px;
  background:rgba(3,17,28,.88);backdrop-filter:blur(18px) saturate(130%);
  box-shadow:0 12px 32px rgba(0,0,0,.24),inset 0 1px 0 rgba(255,255,255,.035);
}
.stTabs [data-baseweb="tab"]{
  position:relative;height:3rem;border:1px solid transparent;border-radius:12px;padding:0 1rem;
  color:#a8c1cf;font-size:.77rem;font-weight:800;letter-spacing:.025em;white-space:nowrap;
  transition:transform .18s ease,color .18s ease,background .18s ease,border-color .18s ease,box-shadow .18s ease;
}
.stTabs [data-baseweb="tab"]:hover{color:#effeff;background:rgba(122,231,244,.075);border-color:rgba(122,231,244,.18);transform:translateY(-1px)}
.stTabs [aria-selected="true"]{
  color:#eaffff!important;background:linear-gradient(135deg,rgba(122,231,244,.19),rgba(63,169,255,.13))!important;
  border-color:rgba(122,231,244,.48)!important;box-shadow:0 8px 24px rgba(18,151,190,.16),inset 0 1px 0 rgba(255,255,255,.08)!important;
}
.stTabs [data-baseweb="tab-highlight"]{display:none}
.stTabs [data-baseweb="tab-panel"]{padding-top:1rem}

/* Contemporary selection-card controls */
.st-key-primary_objective_tiles .stButton>button,
.st-key-intensity_tiles .stButton>button{
  min-height:5.7rem!important;text-align:left!important;justify-content:flex-start!important;
  white-space:pre-line!important;line-height:1.34!important;padding:.9rem 1rem!important;
  background:linear-gradient(145deg,rgba(10,36,53,.78),rgba(6,24,38,.72))!important;
  color:#dffbff!important;border:1px solid rgba(122,231,244,.18)!important;border-radius:16px!important;
  box-shadow:0 10px 25px rgba(0,0,0,.16),inset 0 1px 0 rgba(255,255,255,.03)!important;
  transition:transform .18s ease,border-color .18s ease,box-shadow .18s ease,background .18s ease!important;
}
.st-key-primary_objective_tiles .stButton>button:hover,
.st-key-intensity_tiles .stButton>button:hover{
  transform:translateY(-2px)!important;border-color:rgba(122,231,244,.55)!important;
  background:linear-gradient(145deg,rgba(13,47,68,.92),rgba(8,31,47,.86))!important;
  box-shadow:0 16px 34px rgba(0,0,0,.24),0 0 24px rgba(122,231,244,.09)!important;
}
.st-key-primary_objective_tiles .stButton>button[kind="primary"],
.st-key-primary_objective_tiles .stButton>button[data-testid="stBaseButton-primary"],
.st-key-intensity_tiles .stButton>button[kind="primary"],
.st-key-intensity_tiles .stButton>button[data-testid="stBaseButton-primary"]{
  color:#efffff!important;background:linear-gradient(145deg,rgba(31,118,148,.40),rgba(20,68,102,.42))!important;
  border-color:#7ae7f4!important;box-shadow:0 0 0 1px rgba(122,231,244,.72),0 12px 30px rgba(16,145,183,.25),inset 0 1px 0 rgba(255,255,255,.12)!important;
}
.st-key-intensity_tiles .stButton>button{min-height:4.85rem!important;text-align:center!important;justify-content:center!important}

/* Refined native fields */
div[data-baseweb="input"]>div,div[data-baseweb="textarea"]>div,div[data-baseweb="select"]>div{
  min-height:3rem;background:rgba(4,22,34,.88)!important;border:1px solid rgba(122,231,244,.20)!important;
  border-radius:13px!important;box-shadow:inset 0 1px 0 rgba(255,255,255,.025);transition:border-color .16s ease,box-shadow .16s ease!important;
}
div[data-baseweb="input"]>div:focus-within,div[data-baseweb="textarea"]>div:focus-within,div[data-baseweb="select"]>div:focus-within{
  border-color:rgba(122,231,244,.72)!important;box-shadow:0 0 0 3px rgba(122,231,244,.10),0 10px 24px rgba(0,0,0,.14)!important;
}
[data-testid="stFileUploaderDropzone"]{padding:1.35rem;border:1px dashed rgba(122,231,244,.52)!important;border-radius:17px!important;background:linear-gradient(145deg,rgba(10,38,56,.68),rgba(5,22,35,.62))!important;transition:.18s ease!important}
[data-testid="stFileUploaderDropzone"]:hover{border-color:#7ae7f4!important;background:rgba(122,231,244,.075)!important;box-shadow:0 0 28px rgba(122,231,244,.08)}
[data-baseweb="tag"]{border-radius:999px!important;padding:.18rem .38rem!important;box-shadow:inset 0 1px 0 rgba(255,255,255,.12)}
[data-testid="stToggle"] label{padding:.28rem .38rem;border-radius:999px;transition:background .16s ease}[data-testid="stToggle"] label:hover{background:rgba(122,231,244,.06)}
button:focus-visible,[role="tab"]:focus-visible,input:focus-visible,textarea:focus-visible{outline:2px solid #7ae7f4!important;outline-offset:3px!important}
...
def compute_pstoa(state):
    ...
    Assessment) under the E-AGPO-HT framework. Every sub-score is computed from this session's
...
def build_pstoa_export(state):
    ...
    export = {
        "framework": "E-AGPO-HT (WinterStorm2030 adaptation)",
        "pi": "Evans, G-A.",
...
| Tab | Use it for |
|---|---|
| Scenario Design | Configure mission, vector, intensity, actors, and doctrine — start here |
| Actor & Alliance Analysis | Review Blue and Red actor baselines, seams, and alliance exposure |
| Strategic Concessions | Test access, resource, and legal-policy concessions |
| Capability Readiness | Monitor the composite gap and five-domain readiness picture |
| Governance Operations | Track scenario tempo and live governance indicators |
| OSINT Intelligence | Review real-event signals and tag evidence into the simulation |
| Cognitive Influence | Assess doctrine-based influence intent and governance pressure |
| Narrative Risk | Classify narrative exposure and recommended response posture |
| Dispute Resolution | Manage the five-stage consultation and arbitration ladder |
| Policy Assurance Lab | Score treaty, policy, and legal-agreement coverage |
        """)
...
def generate_objective_guidance(state):
    ...
    if primary == "build_evidence_base":
        ...
        return f"Tag real OSINT events into the Article 4 Stress Test Log: {tagged}/3 so far. Go to the OSINT Intelligence tab and use \"→ Article 4 Log\" on a real (not cached) event."
...
if is_dm:
    st.caption("Decision-Maker mode: the Arbitration Track (a procedural escalation ladder), Cognitive Warfare, Narrative Analysis, and Governance Lab are hidden. "
               "Scenario Design is editable in both modes. Switch to Decision-Support for the full depth toolkit.")
...
tab_names = ["Scenario Design", "Actor & Alliance Analysis", "Strategic Concessions", "Capability Readiness",
             "Governance Operations", "OSINT Intelligence"]
if not is_dm:
    tab_names += ["Cognitive Influence", "Narrative Risk", "Dispute Resolution", "Policy Assurance Lab"]
...
with tab_scenario:
    ...
    if "selected_primary_objective" not in st.session_state:
        st.session_state["selected_primary_objective"] = default_pri
    with st.container(key="primary_objective_tiles"):
        objective_cols = st.columns(3)
        for i, objective_key in enumerate(pri_keys):
            objective = PRIMARY_OBJECTIVES[objective_key]
            selected = st.session_state["selected_primary_objective"] == objective_key
            with objective_cols[i % 3]:
                if st.button(
                    f"{objective['label']}\n{'SELECTED' if selected else 'Select mission objective'}",
                    key=f"objective_tile_{objective_key}",
                    type="primary" if selected else "secondary",
                    use_container_width=True,
                    help=objective["description"],
                ):
                    st.session_state["selected_primary_objective"] = objective_key
                    st.rerun()
    primary_objective = st.session_state["selected_primary_objective"]
    st.markdown(
        f"<div class='ws-callout-panel'><strong style='color:#edf7fa'>Selected mission:</strong> "
        f"{PRIMARY_OBJECTIVES[primary_objective]['label']}<br><span style='font-size:.86rem'>"
        f"{PRIMARY_OBJECTIVES[primary_objective]['description']}</span></div>",
        unsafe_allow_html=True,
    )
    ...
    default_intensity = state.get("intensity", "medium") if state.get("intensity", "medium") in int_keys else int_keys[0]
    if "selected_intensity_choice" not in st.session_state:
        st.session_state["selected_intensity_choice"] = default_intensity
    with st.container(key="intensity_tiles"):
        intensity_cols = st.columns(len(int_keys))
        for i, intensity_key in enumerate(int_keys):
            intensity = INTENSITY_LEVELS[intensity_key]
            selected = st.session_state["selected_intensity_choice"] == intensity_key
            with intensity_cols[i]:
                if st.button(
                    f"{intensity['label']}\n{intensity['sub']}",
                    key=f"intensity_tile_{intensity_key}",
                    type="primary" if selected else "secondary",
                    use_container_width=True,
                    help=intensity["help"],
                ):
                    st.session_state["selected_intensity_choice"] = intensity_key
                    st.rerun()
    selected_intensity = st.session_state["selected_intensity_choice"]
    ...
    if st.button("🚀 Execute Players Move", type="primary"):
        ...
        st.info("🧭 **Next:** trigger your opening move in the Strategic Concessions tab, or click **End Turn** (top of page) to see the Agentic AI's first Red Team move.")
        st.rerun()
...
if not is_dm:
    ...
    with tab_govlab:
        ...
        upload_mode = st.segmented_control(
            "Document input",
            ["Paste document text", "Upload document file"],
            default="Paste document text",
            key="govlab_input_mode",
        )
        if upload_mode is None:
            upload_mode = "Paste document text"
        doc_title = st.text_input("Document title", placeholder="e.g. North Atlantic Treaty (1949)")
        doc_text = ""
        if upload_mode == "Paste document text":
        ...
        st.write("WinterStorm2030's adaptation of Auracelle Charlie's PSTOA scoring under the E-AGPO-HT framework — every dimension below is computed from this session's real data, not copied from Charlie's own values.")
        ...
        with st.expander("Session Metadata"):
            meta_rows = [
                ["Framework", "E-AGPO-HT (WinterStorm2030 adaptation)"], ["PI", "Evans, G-A."], ["Institution", "Bath Spa University"],
