...
:root{
  --bg0:#0b0f11;--bg1:#12181c;--bg2:#1b2329;--panel:#171d22;--panel2:#1f282e;--panel3:#242e35;
  --ink:#e9ece7;--muted:#a8b0a7;--muted2:#7f8d82;--line:#33413b;--line2:#54655d;
  --green:#9ec77a;--green2:#c3d6a1;--amber:#d3bd7a;--red:#d68b7f;--blue:#86a6b8;
}
html,body,[class*="css"]{font-family:'Inter',sans-serif;background:var(--bg0)}
.stApp{
  color:var(--ink);
  background:
    linear-gradient(180deg, rgba(255,255,255,.018), rgba(255,255,255,0) 10%),
    linear-gradient(135deg,#0a0e10 0%, #101519 38%, #171d21 70%, #1d252a 100%);
}
.stApp:before{
...
.block-container{max-width:1560px;padding-top:.9rem;padding-bottom:3rem;position:relative}
header[data-testid="stHeader"]{background:transparent}#MainMenu,footer{visibility:hidden}
h1,h2,h3,h4{font-family:'Rajdhani',sans-serif;color:var(--ink);letter-spacing:.02em}

.ws-toolbar{display:flex;justify-content:space-between;align-items:center;gap:1rem;flex-wrap:wrap;padding:.6rem .9rem;margin:0 0 .85rem;border:1px solid var(--line);background:linear-gradient(180deg,#161d21,#101518);border-radius:8px;box-shadow:0 8px 22px rgba(0,0,0,.22), inset 0 1px 0 rgba(255,255,255,.03)}
.ws-toolbar-left,.ws-toolbar-right{display:flex;gap:.45rem;flex-wrap:wrap;align-items:center}
.ws-toolchip{padding:.3rem .55rem;border:1px solid #3d4a42;border-radius:4px;background:#20272c;color:var(--green2);font-size:.7rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase}
.ws-toolchip.alt{color:#d9e2d1;background:#262f34}
.ws-toolchip.warn{color:var(--amber)}

.ws-hero,.ws-panel,.ws-callout-panel,.ws-brief,.ws-domain-card,.ws-mini-card,.ws-mode-note{
  position:relative;overflow:hidden;border:1px solid var(--line);border-radius:10px;background:linear-gradient(180deg,var(--panel2),var(--panel));
  box-shadow:0 10px 26px rgba(0,0,0,.24), inset 0 1px 0 rgba(255,255,255,.03);
}
.ws-hero{padding:1.15rem 1.2rem 1.05rem;border-color:var(--line2);margin:.15rem 0 1rem}
.ws-hero:before,.ws-panel:before,.ws-brief:before,.ws-domain-card:before{content:"";position:absolute;left:0;right:0;top:0;height:3px;background:linear-gradient(90deg,var(--green),transparent 65%)}
.ws-kicker{color:var(--green2);font-size:.72rem;font-weight:800;letter-spacing:.16em;text-transform:uppercase}
.ws-title{font-family:'Rajdhani',sans-serif;font-size:clamp(2.1rem,3.6vw,3.4rem);line-height:.94;font-weight:700;margin:.24rem 0 .38rem}
.ws-subtitle{color:var(--muted);font-size:.93rem;max-width:960px;line-height:1.42}
.ws-badges{display:flex;gap:.42rem;flex-wrap:wrap;margin-top:.75rem}
.ws-badge{padding:.33rem .58rem;border:1px solid #445149;border-radius:4px;background:#21292e;color:#edf1ea;font-size:.7rem;font-weight:700;letter-spacing:.06em;text-transform:uppercase}
.ws-section-label{margin:.35rem 0 .68rem;color:var(--green2);font-size:.72rem;font-weight:800;letter-spacing:.18em;text-transform:uppercase}
.ws-status{display:inline-flex;align-items:center;gap:.42rem;flex-wrap:wrap;font-size:.73rem;color:var(--muted)}
.ws-dot{width:8px;height:8px;border-radius:2px;display:inline-block;background:var(--green);box-shadow:0 0 0 1px rgba(0,0,0,.22)}
.ws-risk-low{color:var(--green2)}.ws-risk-med{color:var(--amber)}.ws-risk-high{color:var(--red)}
.ws-callout-panel{padding:.95rem 1rem;border-left:4px solid var(--green);background:linear-gradient(180deg,#1d252a,#171d22);color:var(--muted)}
.ws-panel{padding:1rem 1.05rem}.ws-brief{padding:1rem 1rem;height:100%}
.ws-brief-title{font-family:'Rajdhani',sans-serif;font-size:1.28rem;font-weight:700;margin-bottom:.3rem}
.ws-brief-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:.58rem;margin-top:.72rem}
.ws-mini-card{padding:.72rem .74rem;border-radius:8px;background:linear-gradient(180deg,#242d33,#1d252a);border:1px solid #344039}
.ws-mini-card small{display:block;color:var(--muted2);font-size:.66rem;text-transform:uppercase;letter-spacing:.12em;margin-bottom:.22rem}
.ws-mini-card strong{display:block;color:var(--ink);font-size:1rem;font-weight:700;line-height:1.22}
.ws-feature-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:.65rem;margin:.68rem 0 .2rem}
.ws-feature{padding:.78rem .8rem;border:1px solid var(--line);border-radius:8px;background:linear-gradient(180deg,#20292e,#1a2126);color:var(--muted);font-size:.8rem}.ws-feature strong{color:var(--ink);display:block;margin-bottom:.22rem;font-weight:700}
.ws-domain-grid{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:.65rem;margin:.75rem 0 1rem}
.ws-domain-card{padding:.74rem .8rem;border-radius:8px;background:linear-gradient(180deg,#20292e,#171d22)}
.ws-domain-card .name{display:block;font-size:.76rem;color:var(--muted);min-height:2.3rem;line-height:1.28}
.ws-domain-card .gap{display:block;font-family:'Rajdhani',sans-serif;font-size:1.95rem;font-weight:700;color:var(--ink);line-height:1.05;margin:.18rem 0}
.ws-domain-card .tag{display:inline-block;padding:.22rem .45rem;border-radius:4px;font-size:.64rem;font-weight:800;letter-spacing:.08em;text-transform:uppercase}
.ws-domain-card.low .tag{background:rgba(158,199,122,.12);color:var(--green2);border:1px solid rgba(158,199,122,.28)}
.ws-domain-card.med .tag{background:rgba(211,189,122,.12);color:var(--amber);border:1px solid rgba(211,189,122,.28)}
.ws-domain-card.high .tag{background:rgba(214,139,127,.12);color:var(--red);border:1px solid rgba(214,139,127,.28)}
.ws-chip-row{display:flex;gap:.45rem;flex-wrap:wrap;margin-bottom:.65rem}.ws-chip{padding:.28rem .52rem;border-radius:4px;border:1px solid var(--line);background:#21292e;color:var(--ink);font-size:.7rem;font-weight:700;text-transform:uppercase;letter-spacing:.05em}
.ws-mode-note{padding:.8rem .9rem;border-radius:8px;background:linear-gradient(180deg,#1c2429,#151b20);margin-bottom:.85rem}

div[data-testid="stMetric"]{background:linear-gradient(180deg,#20292e,#171d22);border:1px solid var(--line);border-radius:10px;padding:.95rem 1rem;min-height:118px;box-shadow:0 8px 20px rgba(0,0,0,.18), inset 0 1px 0 rgba(255,255,255,.02)}
div[data-testid="stMetricLabel"]{color:var(--muted2);font-size:.7rem;text-transform:uppercase;letter-spacing:.11em;font-weight:800}div[data-testid="stMetricValue"]{color:var(--ink);font-family:'Rajdhani',sans-serif;font-weight:700;font-size:1.95rem}
.stButton>button,.stFormSubmitButton>button{min-height:2.62rem;background:linear-gradient(180deg,#98b97a,#7b9e5c)!important;color:#0f1411!important;font-weight:800!important;border:1px solid #a8bf8e!important;border-radius:6px!important;padding:.62rem .95rem!important;box-shadow:0 6px 14px rgba(0,0,0,.22), inset 0 1px 0 rgba(255,255,255,.18)!important;transition:.16s ease!important}
.stButton>button:hover,.stFormSubmitButton>button:hover{filter:brightness(1.04)}.stButton>button:active,.stFormSubmitButton>button:active{transform:translateY(1px)}
div[data-baseweb="input"]>div,div[data-baseweb="textarea"]>div,div[data-baseweb="select"]>div{background:#1a2126!important;border-color:#3a4640!important;border-radius:6px!important}input,textarea{color:var(--ink)!important}.stTextInput label,.stTextArea label,.stSelectbox label,.stRadio label,.stToggle label{color:var(--muted)!important;font-weight:700!important}
.stTabs [data-baseweb="tab-list"]{gap:.34rem;padding:.34rem;border:1px solid var(--line);border-radius:8px;background:#151b20;overflow-x:auto}.stTabs [data-baseweb="tab"]{height:2.7rem;border-radius:5px;padding:0 .9rem;color:var(--muted);font-weight:700}.stTabs [aria-selected="true"]{background:#263036!important;color:var(--green2)!important}.stTabs [data-baseweb="tab-highlight"]{background-color:var(--green)}
div[data-testid="stVerticalBlockBorderWrapper"]{border-color:var(--line)!important;border-radius:10px!important;background:rgba(25,31,36,.56)}div[data-testid="stExpander"]{border:1px solid var(--line);border-radius:8px;background:#181f24}div[data-testid="stAlert"]{border-radius:8px;border-width:1px}[data-testid="stDataFrame"]{border:1px solid var(--line);border-radius:10px;overflow:hidden}[data-testid="stDataFrame"] *{font-size:.88rem}
hr{border-color:rgba(129,143,133,.18)!important}.stProgress>div>div>div>div{background:linear-gradient(90deg,#9ec77a,#d3bd7a,#d68b7f)}.stProgress>div>div{background:rgba(255,255,255,.08)}
...
@media(max-width:900px){.block-container{padding-left:.9rem;padding-right:.9rem}.ws-feature-grid,.ws-domain-grid,.ws-brief-grid{grid-template-columns:1fr}.ws-title{font-size:2.45rem}.ws-toolbar{padding:.55rem .7rem}}
</style>
...
def render_mak_toolbar():
    st.markdown(
        """
        <div class="ws-toolbar">
          <div class="ws-toolbar-left">
            <span class="ws-toolchip">Synthetic Environment</span>
            <span class="ws-toolchip alt">Runtime Scenario Control</span>
            <span class="ws-toolchip alt">2D / 3D Ops View</span>
            <span class="ws-toolchip alt">Computer Generated Forces</span>
          </div>
          <div class="ws-toolbar-right">
            <span class="ws-toolchip warn">Instructor / Operator</span>
            <span class="ws-toolchip">MAK-ONE-Inspired UI</span>
          </div>
        </div>
        """,
        unsafe_allow_html=True,
...
def render_briefing_panel(state, callsign, posture, posture_class):
    ...
    st.markdown(
        ...
        <div class="ws-brief">
          ...
          <div class="ws-brief-title">Operator Situation Snapshot</div>
...
if not st.session_state["authenticated"]:
    ...
    st.markdown("""
    <div class="ws-hero">
      <div class="ws-kicker">Auracelle AI Governance Labs · MAK-ONE-Inspired NATO Arctic High North Interface</div>
      <div class="ws-title">WINTERSTORM<span style="color:#7ae7f4">2030</span></div>
      <div class="ws-subtitle">A MAK-ONE-inspired synthetic-environment interface for asynchronous governance wargaming—cleaner, more tactical, and operator-focused for Arctic High North decision-testing.</div>
      <div class="ws-badges">
...
with hero_left:
    st.markdown(f"""
    <div class="ws-hero">
      <div class="ws-kicker">NATO STO SAS-219 · High North Scenarios for Wargaming & Analysis · MAK-ONE-Inspired Aesthetic</div>
      <div class="ws-title" style="font-size:2.85rem">WINTERSTORM<span style="color:#7ae7f4">2030</span></div>
      <div class="ws-subtitle"><strong style="color:#edf7fa">{scenario_label}</strong><br>Synthetic environment control view · governance capacity · grey-zone competition · strategic concession analysis across the Arctic High North.</div>
      <div class="ws-badges">
