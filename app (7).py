...
if not st.session_state["authenticated"]:
    ...
    <div class="ws-feature-grid">
      ...
      <div class="ws-feature"><strong>Evidence Base</strong>Every score, gap, and finding traces back to real-time OSINT data or historical treaty precedent — not assumption. Where that evidence points is for the reviewing body to decide.</div>
...
with st.expander("ℹ️ Instructions — Architecture & Navigation Guide", expanded=False):
    st.markdown("""
...
if state.get("turn_action_taken"):
    st.markdown(
        ...
        f"✅ <strong style='color:var(--green);'>Move logged for Turn {state['turn']}.</strong> Ready to End Turn when you are — that will generate the next Red Team move and advance to Turn {state['turn']+1}.</div>",
        unsafe_allow_html=True)
else:
    st.markdown(
        ...
        f"🎯 <strong style='color:#3fa9ff;'>It's your move — Turn {state['turn']}.</strong> Take an action (Concession Engine, OSINT tag, Cognitive Warfare, Narrative Analysis, or Arbitration Track) before clicking End Turn again. "
        f"End Turn always works, but nothing else has happened yet this turn.</div>",
        unsafe_allow_html=True)
